...
# Title: 02_deployment-timeframes
# Description: Calculate camera deployment operating time ranges by day
...
# Camera start and end times (from metadata provided for cameras deployed during 2013 to 2018)
# Note that this is for data tagged prior to use of the WildTrax platform
df_old_cam_ranges <- read_csv("./data/lookup/abmi-cmu-other_2013-2018_startend.csv")

# Field of view END/START lookup
df_fov_es <- read_csv("./data/lookup/abmi-cmu_all-years_end-start-field-of-view.csv")
...
df_new_cam_ranges <- df_all %>%
  ...
  # Fix a couple obvious mistakes in the database
...
df_inter_pairs_extra <- read_csv("data/lookup/cmu-2018_intermediate-end-start-pairs.csv") %>%
...
write_csv(df_tbd_summary, paste0(processed, "abmi-cmu_all-years_tbd-summary.csv"))
