...
# Title: 03_effective-detection-distance
# Description: Modeling of effective detection distance (EDD) by species, habitat, and season
...
# Lure lookup
df_lure <- read_csv("./data/lookup/abmi-cmu_all-years_lure.csv")

# Habitat/Human Footprint at the camera point and associated detection distance category
df_veghf <- read_csv("./data/lookup/abmi-cmu_all-years_veghf-soilhf-detdistveg.csv")

# EDD modeling species groups
df_edd_groups <- read_csv("./data/lookup/species-distance-groups.csv")

# Habitat/Human Footprint groupings to try in modeling
df_veghf_groups <- read_csv("./data/lookup/veg-pole-distance.csv")

# Pole position tags
df_pole <- read_csv("./data/lookup/abmi_all-years_pole-position.csv")
...
# Step 1: Create data frame of pole position info, season, and habitat to use in EDD modeling
...
# Save intermediate results for downstream use
# Use `data/processed` - user can change if they wish. This folder is in the .gitignore.

processed <- "./data/processed/"
write_csv(df_pole_veghf, paste0(processed, "pole_position_veghf.csv"))
...
for (sp in 1:length(SpTable)) {
  ...
  fname<-paste0(processed, "Detection distance ",SpTable[sp],".jpg",sep="")
  ...
  fname<-paste0(processed, "Detection distance models ",SpTable[sp],".rdata",sep="")
...
fname.models <- paste0(processed, "Detection distance models ")
...
save(dd, dd.lci, dd.uci, SpTable, site.list,
     file = paste0(processed, "Detection distances by site species and season.rdata"))
